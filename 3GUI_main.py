...
class MainWindow(QWidget, form_class):
    ...
    def __init__(self):
        ...
        #self.detection.clicked.connect(self.controlTimer)
        self.controlTimer()
    ...
    def Detection(self):
        ...
        if not self.timer2.isActive():
            self.log_browser.setText("[INFO] detecting video stream thread...")
    ...
    def faceRegistration(self):
        self.log_browser.setText("faceRegistration")
        print("face")
        for i in range(10):
            ret, frame = self.cap.read()
            frame = cv2.flip(frame, 2)
            ...
            folder_name = "/home/qisens/facedetection/dataset/known"
            image.save(folder_name + '/' + image_name)
        print("Training KNN classifier...")
        train("dataset", model_save_path="trained_knn_model.clf", n_neighbors=2)
